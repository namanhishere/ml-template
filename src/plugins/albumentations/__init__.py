...
def get_augmentations(config: dict, phase: str = "train"):
    ...
    try:
        ...
        logging.getLogger("ai-ml-template").warning(
            "albumentations not installed, falling back to torchvision transforms"
        )
...
class ToTensorV2_albu:
    ...
    def __call__(self, **kwargs):
        ...
        return {
            k: torch.from_numpy(v).permute(2, 0, 1) if isinstance(v, np.ndarray) and v.ndim == 3 else v
